...
void OGLRenderer::SetShaderLight(const Light& light)
{
	...
	glUniform4fv(glGetUniformLocation(currentShader->GetProgram(), "specColour"), 1, (float*)&light.GetSpecColour());
	glUniform1f(glGetUniformLocation(currentShader->GetProgram(), "lightRadius"), light.GetRadius());
	glUniform1i(glGetUniformLocation(currentShader->GetProgram(), "lightType"), (int)light.Get_Type());
}

void OGLRenderer::SetShaderLight(const DirLight& light)
{
